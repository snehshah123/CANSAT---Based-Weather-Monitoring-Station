...
class WeatherApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.mqtt_running = False  # MQTT state tracker
        ...
        self.ui.MQTT_SW.clicked.connect(self.toggle_mqtt)
    ...
    def start_mqtt(self):
        ...
        if not self.mqtt_running:
            ...
            self.mqtt_thread = threading.Thread(target=self.publish_mqtt, daemon=True)
    ...
    def stop_recording(self):
        """Stop recording and MQTT publishing."""
        self.recording = False
        self.stop_mqtt()  # Stop MQTT when RESET is clicked
    ...
    def toggle_mqtt(self):
        """Start or stop MQTT publishing when the MQTT button is clicked."""
        if self.mqtt_running:
            self.stop_mqtt()
        elif self.recording:  # Only start MQTT if RUN has been clicked
            self.start_mqtt()
        else:
            print("Press RUN first to start recording sensor data.")
            
    def publish_mqtt(self):
        """Continuously reads sensor data and publishes to MQTT."""
        self.mqtt_client.connect(HOST, PORT, 60)
        self.mqtt_client.loop_start()

        while self.mqtt_running:
            try:
                sensor_data = {"time": time.strftime("%H:%M:%S")}

                # If BMP280 is selected, add temperature, pressure, altitude
                if self.show_bmp280:
                    sensor_data.update({
                        "temperature": round(bmp.temperature, 2),
                        "pressure": round(bmp.pressure, 2),
                ...
                # If IMU is selected, add roll, pitch, yaw
                if self.show_imu:
                    accel_x, accel_y, accel_z, gyro_z = read_imu_data()
                    roll = round(math.atan2(accel_y, accel_z) * 180 / math.pi, 2)
                    pitch = round(math.atan2(-accel_x, (accel_y**2 + accel_z**2) ** 0.5) * 180 / math.pi, 2)
                ...
                # Publish only if at least one sensor is selected
                self.mqtt_client.publish('bmp280/sensordata', json.dumps(sensor_data), qos=1)

                time.sleep(2)  # Match the INTERVAL in mqtt.py
